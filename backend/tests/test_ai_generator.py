...
class TestAIGeneratorGenerateResponse:
    ...
    def test_generate_response_with_tools_no_use(self, mock_anthropic_class):
        ...
        mock_client = mock_anthropic_class.return_value
        mock_client.messages.create.return_value = create_text_response("Answer without tools")
    ...
    @patch('ai_generator.anthropic.Anthropic')
    def test_generate_response_triggers_tool_use(self, mock_anthropic_class, mock_tool_manager):
        ...
        result = generator.generate_response("What is ML?", tools=tools, tool_manager=mock_tool_manager)
    ...
    def test_generate_response_with_conversation_history(self, mock_anthropic_class):
        ...
        mock_client = mock_anthropic_class.return_value
        mock_client.messages.create.return_value = create_text_response("Response with context")
    ...
    def test_generate_response_missing_api_key(self, mock_anthropic_class):
        ...
        mock_client = mock_anthropic_class.return_value
        mock_client.messages.create.side_effect = Exception("Authentication failed: API key required")
        ...
        assert "Authentication" in str(exc_info.value) or "API key" in str(exc_info.value)
...
class TestAIGeneratorToolExecution:
    ...
    @patch('ai_generator.anthropic.Anthropic')
    def test_handle_tool_execution_single_tool(self, mock_anthropic_class, mock_tool_manager):
    ...
    @patch('ai_generator.anthropic.Anthropic')
    def test_handle_tool_execution_tool_returns_error(self, mock_anthropic_class, mock_tool_manager):
    ...
    @patch('ai_generator.anthropic.Anthropic')
    def test_handle_tool_execution_tool_raises_exception(self, mock_anthropic_class, mock_tool_manager):
        """
        ...
        mock_tool_manager.execute_tool.side_effect = Exception("Tool crashed unexpectedly")
    ...
    @patch('ai_generator.anthropic.Anthropic')
    def test_handle_tool_execution_message_format(self, mock_anthropic_class, mock_tool_manager):
        """Test that tool results are formatted correctly for the API."""
        from ai_generator import AIGenerator

        mock_client = mock_anthropic_class.return_value
        mock_client.messages.create.side_effect = [
            create_tool_use_response("search_course_content", {"query": "test"}, tool_id="tool_abc"),
...
class TestAIGeneratorSequentialToolCalling:
    ...
    def test_two_sequential_tool_calls(self, mock_anthropic_class, mock_tool_manager):
        ...
        mock_client.messages.create.side_effect = [
            create_tool_use_response("get_course_outline", {"course_title": "AI Course"}, tool_id="t1"),
            create_tool_use_response("search_course_content", {"query": "machine learning"}, tool_id="t2"),
            create_text_response("Final answer after two tool calls")
        ]

        generator = AIGenerator("test-key", "claude-sonnet-4-20250514")
        tools = [
            {"name": "get_course_outline", "description": "Get outline", "input_schema": {"type": "object"}},
            {"name": "search_course_content", "description": "Search", "input_schema": {"type": "object"}}
        ]
        result = generator.generate_response("Query", tools=tools, tool_manager=mock_tool_manager)
